...
% torus shape, interior source RHS (known solution) for now.
% Barnett 10/24/18. code inside loop from waveeqn_tmarch.m
...
wc = 0;  % surf modulation ampl (0 for plain torus)
...
o.p = 4;  % p = spatial "order" (G-L nodes per panel side)
m = 2;    % control time interp order (order actually m+2)
wpred = extrap(m);   % (m+2)th order extrapolation row vector for prediction

al = 1.0;    % repr mixing alpha
nam = sprintf('wt_stab_p%d_m%d',o.p,m);

% define dx and dt parameter plane...
nps = 6:6:36;   % num panels on major torus loop (multiples of 3 best)
...
pcs = [2,3,4,8,-1];     % pred-corr steps (1,2,..;-1=inf)
corrshift = 0.25;
...
for i=1:numel(nps)    % ======================= MAIN DX LOOP
  ...
  [bytes{i} est_times{i} cpu_times{i} cpu_usages{i} labelstrings{i} labeltimes{i}] = memorygraph('get');
