...
% Wider pulse, to aim for more than 4 digits :)
...
s0 = 0.15;         % was 0.12.  closely tied to dt. Error ~ exp(-3*(dt/s0)^2)
...
o.nr = 2*o.p+4; o.nt = 2*o.nr;  % aux quad orders: radial and angular nodes

nps = 6:3:12;    % convergence
...
for i=1:numel(nps)         % ======================== MAIN DX LOOP ========
  ...
  dt = 1.0 * h;      % choose timestep!  (note want small to capture pulse)
  ...
  dx=0.2; gx=-2:dx:2; gz = -1:dx:2; [xx zz] = meshgrid(gx,gz);  % was dx=.05
  ...
  % check at point... (x0,0,z0) and t0 (using interpolation off dt-grid to t0)
  t0=3.5;
  jx = 9; jz = 10; % for dx=.2;   was@dx=.05: jx=37; jz=41;
  j=jz+numel(gz)*(jx-1); x0=xx(j); z0=zz(j);  % one spatial test pt
  fprintf('test pt x0=(%g,%g,%g)\n',x0,0,z0);
  signal = utot(j,:);   % utot at single x0
  [jmax jmin w] = interpmat(-t0,dt,m);   % use order m+2 interp
  utott0 = dot(w(end:-1:1),signal(-jmax:-jmin));  % guessing offset and that w goes backwards in time!
  fprintf('dt=%g,m=%d,np=%d,p=%d,nr=%d: \t u(x0,%g) = %.9f\n',dt,m,so.np,o.p,o.nr,t0,utott0)
...
%  save scattBVPconv_wider_big_dt1.0h.mat   % re-save after every dx surf discr choice
  
end                   % =======================

%memorygraph('done');

% check convergence after runs done: (at any single index j in the utot slice)
clear utes
tes = 1:0.1:9; utes = nan(numel(tes),numel(run));   % reinterp to reg t grid tes
for ru=1:numel(run), r=run{ru};    % loop over diff np runs
  signal = r.utot(j,:);   % utot at x0, on that run's t-grid
  [jmax jmin w] = interpmat(-tes,r.dt,m);    % use order m+2 interp
  utes(:,ru) =  w(:,end:-1:1) * signal(-jmax:-jmin)';
  hs(ru) = r.h;
end
figure; subplot(2,1,1); plot(tes,utes,'-'); title('u_{tot}(x0,t) on same t-grid');
xlabel('t'); ylabel('u_{tot}');
subplot(2,1,2); semilogy(tes,abs(utes(:,end)-utes(:,end-1)),'-'); title('err btw last two runs')
xlabel('t'); ylabel('u ptwise err');
