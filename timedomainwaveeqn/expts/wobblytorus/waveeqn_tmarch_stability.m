...
nam = sprintf('stab_wtorus_p%d_m%d_pulse_al%g_npupto12',o.p,m,al);

% define dx and dt parameter plane...
nps = [3 6 9 12];   % num panels on major torus loop (multiples of 3 best)
dts = [0.05 0.06 0.07 0.085 0.1 0.12 0.15 0.2 0.3 0.4 0.5];  % dt timesteps
maxsteps=max(ceil(Ttot./dts));

% define expt runs for each dx-dt pair...
betas = [0.5 1 2 4];    % beta repr mixing params
pcs = [1,2,3,4,-1];     % pred-corr steps (1,2,..;-1=inf)
...
for i=1:numel(nps)    % ======================= MAIN DX LOOP
  ...
  for j=1:numel(dts), dt=dts(j);    % --------------------- MAIN DT LOOP
    ...
    parfor e=1:ne         % if parfor, restricts outputs to be 1d in e
      ...
      mo = []; mo.verb = 1;   % opts for timestepping, then run it:
