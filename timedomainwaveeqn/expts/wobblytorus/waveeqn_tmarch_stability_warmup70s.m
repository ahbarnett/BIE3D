...
nam = sprintf('stab_wtorus_p%d_m%d_pulse_al%g_np21',o.p,m,al);

% define dx and dt parameter plane...
nps = [9]; %[21 24]; %[15 18];
%nps = [6 9 12 18 24 30];   % num panels on major torus loop (multiples of 3 best)
dts = [0.2 0.4]; %[0.03 0.04 0.05 0.06 0.07 0.085 0.1 0.12 0.15 0.2 0.3 0.4 0.5];  % dt timesteps
maxsteps=max(ceil(Ttot./dts));

% define expt runs for each dx-dt pair...
betas = [0.5 1]; %[0.5 1 2 4];    % beta repr mixing params
pcs = [1,-1]; %[1,2,3,4,-1];     % pred-corr steps (1,2,..;-1=inf)
